...
S_NM_IC_DERIV_PLT = 'ICDerivPlot'

R04 = 4

# --- INPUT -------------------------------------------------------------------
# --- flow control ------------------------------------------------------------
doPlotDerivIC = True            # True / False
...
dPairsICP = dTupIn
nmICDerivPlt = S_NM_IC_DERIV_PLT    # name prefix of the IC component plot
wdthBar = 0.2                       # width of single bars
wdthGrp = 0.75                      # width of bar group
degRotXLbl = 90                     # degree rotation of x-labels

# --- names and paths of files and dirs ---------------------------------------
s1, s2, s3, s4, s5, s6 = 'ICDeriv', 'BinOp', 'MetD', 'PhoD', 'DvSD', 'AllD'
dSFIn_ICDeriv = {sGT: (s1 + S_USC*2 + s2 + S_USC + s3 + S_USC + s5 + S_USC +
                       sGT + S_USC + s6 + S_USC + s4 + S_USC + s5 + S_USC +
                       sGT + S_USC + s6) for sGT in L_S_GT}
sFOutICP = nmICDerivPlt
...
dPFIn_ICDeriv = {sGT: os.path.join(pInCSV, dSFIn_ICDeriv[sGT] + S_DOT + S_CSV)
                 for sGT in L_S_GT}
